...
from decompositions import index_decomposition
import argparse
import torch.optim.lr_scheduler as lr_scheduler
import utils.pickler as pickler
import pickle
import wandb
from genindices import index_selection
import os
import eco2ai
...
def main(numEpochs, batch_size, device, net, 
    ...
    elif model_name.lower() == "efficientnet-b5":
        ...
        if (epoch) % selection_iter == 0:
            if warm_start and selection == 0:
                ...
                if not imgntselloader:
                    ...
                    indices = index_selection(main_trainloader, data3, net, 
                                              clone_dict, batch_size, fraction, 
                                              selection_iter, numEpochs, device, dataset_name)
                else:
                    
                    indices = index_selection(imgntselloader, data3, net,
                                             clone_dict, batch_size, fraction,
                                             selection_iter, numEpochs, device, dataset_name)
...
if __name__ == '__main__':
        ...
        if args.save_pickle:
            ...
            if os.path.exists(file):
                ...
                if args.dataset.lower() != "imagenet":
                    V = index_decomposition(trainloader, args.batch_size, device=args.device, decomp_type=args.decomp)
                    data3 = V
                else:
                    ...
                    V = index_decomposition(imgntselloader, args.batch_size, device=args.device, decomp_type=args.decomp)
                    data3 = V
    ...
    main(args.numEpochs, args.batch_size, args.device, net, args.model, args.dataset, trainloader, valloader, trainset, data3, args.optimizer, args.lr, args.weight_decay, args.grad_clip, args.fraction, args.select_iter, args.warm_start, imgntselloader)
    tracker.stop()
